...
export class ForumCategoryComponent implements OnInit {
  ...
  getCategories(){
    ...
    const categorySubscr = this.cbfService.getForumCategory(this.accessToken)
    .subscribe({
      next: (response: any) => {
        let result = response   
  ...
  saveCategory() {
    ...
    regData.append('name', this.categoryRegistration.get('name')?.value)
    regData.append('description', this.categoryRegistration.get('description')?.value)
    regData.append('modified_by', this.user_id.toString())

    const regsterSubscr = this.cbfService.registerForumCategory(regData, this.accessToken)
  ...
  updateCategoryAction(data: any) {
    ...
    const updateSubscr = this.cbfService.updateForumCategory(updateData, catId,  this.accessToken)
  ...
  deleteCategoryAction(data: any) {
    ...
    const delSubscr = this.cbfService.updateForumCategory(delData, catId, this.accessToken)
